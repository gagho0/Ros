...
class action_client(Node):
    ...
    def feedback_callback(self, msg: Fibonacci):
        feedback: Fibonacci.Feedback =msg,feedback
        self.get_logger().info(f"feedback.temp_seq")


    def goal_response_callback(self, future: Future):
        #첫 goal을 주고 난 다음의 response
        goal_handle: ClientGoalHandle = futre.result() #type:ignore
        if not goal_handle.accepted:
            self.get_logger().info("goal rejected")
            return
        self.get_logger().info("goal accepted")
        #result가 왔을떄의 callback을 등록
        self.get_result_future = goal_handle.get_result_async()
        self.get_result_future.add_done_callback(self.get_result_callback)
        self.get_logger().info("end of the goal response callback function")
        pass

    def feedback_callback(self,msg: Fibonacci.Feedback):
        self.get_logger().info(f"{msg.temp_seq}")
